...
internal fun largemouthPlan(
    ...
    val moodClause = when {
        w.heavyRain -> " A downpour's muddying the water, so slow down, hug cover, and throw something they can find in the murk."
        w.lightRain -> " Rain's falling. It dims the light and washes food into the shallows, so they're up and feeding. Get on it."
        mood == Mood.FEEDING -> " A front's moving in and the pressure's falling, so they're feeding hard right now, so get out ahead of it."
        mood == Mood.TOUGH -> " But it's a bright, high-pressure day behind a front, so they're sluggish and pinned to cover, so slow way down."
    ...
    val tacticLine = if (w.heavyRain) {
        ...
        BassPhase.COLD, BassPhase.LATE_FALL ->
            "Drag a jig or soft-plastic slow on the bottom, hop a blade bait, or twitch a jerkbait with long pauses, keeping it right in their face."
    }
    ...
    val (whereBrief, whereMore) = when (phase) {
        ...
        BassPhase.POSTSPAWN -> Pair(
            ...
            "Check docks, points, and shallow cover between the spawning flats and deeper water. They also raid bluegill beds now, so wherever you find bluegill, bass aren't far behind. They can be moody, so you may have to work for bites.",
        )
    ...
    val whenBrief = buildString {
        ...
        when (mood) {
            Mood.FEEDING -> append("A storm front is coming, so get out now, before it hits. The hours right before bad weather are the best feeding time of the year.")
    ...
    val howMore = buildString {
        when (mood) {
            Mood.TOUGH -> append("Because it's bright after a front, pick apart the thickest shade and cover, because the sun drives bass tight and they won't move far to eat. ")
            ...
            Mood.STEADY -> if (w.clear) append("It's clear and calm, so the fish are spooky, so use lighter line, natural colors, and longer casts. ") else Unit
        ...
        append(" When you catch one, slow down and pick that spot apart, because bass group up, so there are usually more.")
    }
